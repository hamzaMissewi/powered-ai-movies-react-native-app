...
function isReactElementWithProps(
  child: ReactNode
): child is ReactElement<{ hint?: ReactNode }> {
...
export function Section({
  ...
  const childrenWithSeparator = React.Children.map(children, (child, index) => {
    ...
    const childProp = child.props as any;

    // Extract onPress from child
    const originalOnPress = childProp?.onPress;
    ...
    if (child.type === RNText || child.type === Text) {
      child = React.cloneElement(child, {
        ...
        ...childProp!,
        onPress: undefined,
        style: [FormFont.default, childProp?.style],
      });

      // Then in your component:
      const hintView = (() => {
        if (!isReactElementWithProps(child) || !childProp.hint) {
          return null;
        }

        return React.Children.map(childProp.hint, (hintChild) => {
          if (!hintChild) return null;
          if (typeof hintChild === "string") {
            return (
      ...
      const symbolView = (() => {
        if (!childProp?.systemImage) {
          return null;
        }

        const symbolProps =
          typeof childProp.systemImage === "string"
            ? { name: childProp.systemImage }
            : childProp.systemImage;
        ...
        return (
          <IconSymbol
            ...
            color={
              symbolProps.color ??
              extractStyle(childProp.style, "color") ??
              AppleColors.label
            }
          />
        );
        ...
        (linkChild) => {
          ...
          if (typeof linkChild === "string") {
            return (
              <RNText
                dynamicTypeRamp="body"
                style={mergedStyles(FormFont.default, childProp)}
              >
                {linkChild}
              </RNText>
            );
      ...
      const hintView = (() => {
        ...
        return React.Children.map(childProp.hint, (child) => {
          // Filter out empty children
          if (!child) {
            return null;
          }
          ...
          <FormItem>
            <HStack>
              ...
              <View style={{ paddingLeft: 12 }}>
                <LinkChevronIcon href={childProp.href} />
              </View>
            </HStack>
          </FormItem>
        ),
